...
'''
    ...
    if prediction[0:10] == "Entailment":
        ...
        pred_label = 1
    elif prediction[0:12] == "Contradiction":
        pred_label = 2
    else:
        pred_label = 0
...
print(f"F1      : {f1['f1']:.4f}")
results = {"model":"deepseek-r1","dataset":"MNLI","num_samples":num_samples,"accuracy": accuracy["accuracy"],"F1":f1['f1']}
write_jsonl( os.path.join(result_dir, "overall_results.jsonl"),[results])
write_jsonl( os.path.join(result_dir, "MNLI.jsonl"),data_entries, 'w')
